...
WHERE departments.department_id = employees.department_id AND department_name = 'IT';

/*Задача 2*/
SELECT first_name, last_name, country_name FROM employees, countries, departments, locations
WHERE departments.department_id = employees.department_id
AND departments.location_id = locations.location_id
AND locations.country_id = countries.country_id;

/*Задача 3*/
SELECT e.last_name AS Фамилимя_Р, DATE(e.hire_date) AS Дата_Р, man.last_name AS Фамилия_М, DATE(man.hire_date) AS Дата_м FROM employees e INNER JOIN
 (SELECT* FROM employees) AS man ON
DATE(e.hire_date)<DATE(man.hire_date) AND e.manager_id=man.employee_id;

/*Задача 4*/
SELECT p.last_name AS Фамилия_Р, p.first_name AS Имя, p.salary AS Оклад FROM employees p
	INNER JOIN (SELECT avg(salary) AS sal FROM employees) AS s ON
	p.salary<s.sal
ORDER BY salary;
