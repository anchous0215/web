...
('Ivanovа-Skladovskаya-Petrova', 'Ivanka', '89167748924', 'ii2@mail.mephi.ru', 60);
SELECT * FROM bd_employees;

/*Задание 1*/
select * from bd_employees 
where phone_number~'^8-\((495|499)\)-\d{3}-\d{2}-\d{2}$';
...
/*Задание 3*/
select last_name, REGEXP_REPLACE(phone_number, '(^.{1})(.*)(.{1}$)', '\1_\3') from bd_employees;

/*Задание 4*/
select * from bd_employees
where REGEXP_COUNT(last_name, 'a|o|u|i|e', 1, 'i')*2 =  REGEXP_COUNT(last_name, 'b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z', 1, 'i');
