...
CASE WHEN job_id LIKE 'SA_REP' THEN 'Торговый представитель'
ELSE CASE WHEN job_id LIKE 'SA_MAN' THEN 'Менеджер по продажам'
ELSE 'Другое' END END AS Должность FROM EMPLOYEES;

/*Задача 4*/
SELECT MAX(salary) AS "Максимальная зарплата", MIN(salary) AS "минимальная зарплата", AVG(salary) AS "средняя зарплата" FROM EMPLOYEES;
